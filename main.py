...
#Create a class called MyAPP that takes App as an argument
class MyApp(App):
  #Use the init method 
  ...
  #Create a function to respond when the translate button is pressed 
  def on_button_pressed(self, widget):

    #Take the user input and transfer to all lowercase letters
    word = self.textinput.get_value()
    word = word.lower()

    #Use an if statement to find the word in the Spanish and French translations
    if word in self.translations:
      ...
      self.error.set_text()
    #If the word is not located in the translations display a message 
    else:
  ...
  def main(self):
    #Open CSV code, use the DictReader, and use a for loop
